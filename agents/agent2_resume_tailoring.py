...
Council integration: when OPENROUTER_API_KEY or ANTHROPIC_API_KEY is set,
the professional summary is generated by the LLM Council for higher quality.
"""
...
class ResumeTailoringAgent:
    ...
    def tailor(self, job: JobPosting) -> Optional[Path]:
        ...
        When an LLM key is configured, the professional summary is generated
        by the LLM Council for higher quality than keyword concatenation.
        """
    ...
    # ------------------------------------------------------------------
    # LLM Council summary generation
    # ------------------------------------------------------------------

    @staticmethod
    def _generate_council_summary(job: JobPosting, keywords: list,
                                  existing_summary: str) -> Optional[str]:
        """
        Use the LLM Council to write a targeted professional summary.

        Returns the synthesized summary string, or None if no LLM is available.
        The council approach produces more natural, role-specific language than
        simple keyword prepending.
        """
        import os
        ...
        try:
            from utils.llm_council import ask_council
        except ImportError:
            return None

        top_kw = ", ".join(k.title() for k in keywords[:8])
        system = (
            "You are an expert resume writer specialising in senior technology and "
            "product management roles in India. Write tight, impactful professional "
            "summaries. No fluff, no clichés. 2-3 sentences max."
        )
        question = (
            f"Write a professional summary for this resume tailored to the job below.\n\n"
            f"Target role: {job.title} at {job.company}\n"
            f"Job description keywords: {top_kw}\n"
            f"Existing summary (for context, do NOT copy verbatim):\n{existing_summary}\n\n"
            f"Requirements:\n"
            f"- 2-3 sentences, ~50-70 words\n"
            f"- Mention the target role and top 3-4 relevant keywords naturally\n"
            f"- Convey 12+ years of product/BA leadership experience\n"
            f"- End with a value proposition sentence\n"
            f"- No 'I' pronoun, no bullet points, plain paragraph text only"
        )
        result = ask_council(question, system=system, max_tokens=150)
        if result and len(result) > 20:
            print(f"[Agent2] Council generated summary ({len(result)} chars).")
            return result.strip()
        return None
    ...
    def _update_summary(self, doc: Document, job: JobPosting, keywords: list[str]) -> Document:
        """
        Replace/prepend the professional summary.

        Priority:
          1. LLM Council generated summary (highest quality, needs API key)
          2. Keyword-prepend fallback (always available)
        """
        ...
        # Try council first
        council_summary = self._generate_council_summary(job, keywords, existing)
        if council_summary:
            if para.runs:
                para.runs[0].text = council_summary
                for run in para.runs[1:]:
                    run.text = ""
            else:
                para.add_run(council_summary)
            return doc

        # Fallback: role-aligned keyword prepend
