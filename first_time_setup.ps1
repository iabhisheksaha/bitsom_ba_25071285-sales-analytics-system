...
#
# HOW TO RUN (copy-paste into PowerShell, no Admin needed):
...
function Store-Secret {
    ...
    if ($existing) {
        $ans = Read-Host "  '$Target' is already stored. Skip? (Y/n)"
        if ($ans -ne 'n') { Write-Host "  Keeping existing $Target." -ForegroundColor Yellow; return }
    }
...
Store-Secret "JobApp_CRED_KEY"           "Your CRED_KEY (Fernet encryption key)"
Store-Secret "JobApp_TELEGRAM_BOT_TOKEN" "Telegram bot token"
Store-Secret "JobApp_TELEGRAM_CHAT_ID"   "Telegram chat ID (your numeric ID)"
...
# Install CredentialManager module if missing
...
if ($LASTEXITCODE -ne 0) { Write-Host "[ERROR] init_credentials.py failed." -ForegroundColor Red; exit 1 }
...
Write-Host "You will receive a Telegram message with the daily summary." -ForegroundColor White
Write-Host ""
Write-Host "If a job site asks for a new login you'll get a Telegram" -ForegroundColor White
Write-Host "message — just reply:  /creds your@email.com:password" -ForegroundColor White
Write-Host "The password is saved automatically for future runs." -ForegroundColor White
Write-Host ""
Write-Host "Useful commands:" -ForegroundColor Cyan
Write-Host "  Run immediately  : Start-ScheduledTask -TaskName 'JobApplicationAgents_DailyRun'"
Write-Host "  View live log    : Get-Content '$JobAppDir\logs\scheduler.log' -Wait"
Write-Host "  Pause the bot    : Disable-ScheduledTask -TaskName 'JobApplicationAgents_DailyRun'"
Write-Host "  Resume the bot   : Enable-ScheduledTask  -TaskName 'JobApplicationAgents_DailyRun'"
